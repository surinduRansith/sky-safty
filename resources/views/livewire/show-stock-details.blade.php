      ...
      <div class="mb-4">
          ...
          @if (count($sizes) > 0)
              ...
              <div class="flex items-center mb-2">
                  <span class="border rounded w-12   p-2 mr-2 {{ $index % 2 == 0 ? 'bg-yellow-200' : 'bg-blue-200' }} text-black">
                      {{ $size['size'] }}
                  </span>
                  <span class="border rounded p-2 mr-2 {{ $index % 2 == 0 ? 'bg-green-200' : 'bg-red-200' }} text-black">
                      {{ $size['quantity'] }}
                  </span>
              </div>
...
<div class="card card-compact bg-base-100 w-[50%] shadow-xl pt-4">
    ...
    <div class="card-body ">
      ...
      <div class="mb-4">
          ...
          <input disabled type="text" id="code" wire:model.defer="code" class="input input-bordered w-full max-w-xs">
      ...
      <div class="mb-4">
          ...
          @foreach ($sizes as $index => $size)
              <div class="flex items-center mb-2">
                <input type="text" wire:model.defer="sizes.{{ $index }}.size" placeholder="Size" class="border rounded p-2 mr-2 w-25">
                <input type="number" wire:model.defer="sizes.{{ $index }}.quantity" placeholder="Quantity" class="border rounded p-2 mr-2 w-25">
                <button type="button" wire:click="removeSize({{ $index }})" class="btn btn-error text-white btn-sm">Remove</button>
            </div>
      ...
      <div class="card-actions justify-end mb-4">
          @foreach ($stocks as $index => $stock)
          <button wire:click="update({{ $stock->id }})" class="bg-blue-500 text-white font-bold py-2 px-4 rounded">
              Create Stock
              <span class="loading loading-spinner text-primary" wire:loading></span>
          </button>
          @endforeach
  
          <button wire:click="canceledit" class="bg-red-500 text-white font-bold py-2 px-4 rounded">Cancel</button>
      </div>
    </div>
  </div>
